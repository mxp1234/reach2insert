#!/usr/bin/env python3
"""
DP-SERL 课程学习训练脚本 (快速切换 + 相对坐标系版本 + 触觉传感器 + 录制功能)
...
- 全程录制触觉数据和相机图像，用于后续可视化分析

录制功能:
- 自动录制 wrist_2 和 top 相机图像
- 自动录制 6D 触觉数据 [Fx, Fy, Fz, Mx, My, Mz]
- 录制机器人位姿、动作等状态信息
- 训练结束时自动保存为 .npz 格式
- 可通过 visualize_recording.py 脚本生成视频

Terminal 1 (Learner):
    export XLA_PYTHON_CLIENT_PREALLOCATE=false
    export XLA_PYTHON_CLIENT_MEM_FRACTION=.2
    python run_dp_serl_curriculum_fast_img-only_tactile-recording.py --learner --exp_name=peg_in_hole_square_III

Terminal 2 (Actor):
    export XLA_PYTHON_CLIENT_PREALLOCATE=false
    export XLA_PYTHON_CLIENT_MEM_FRACTION=.2
    python run_dp_serl_curriculum_fast_img-only_tactile-recording.py --actor --exp_name=peg_in_hole_square_III --ip=localhost

录制文件保存位置:
    {checkpoint_path}/recordings/recording_YYYYMMDD_HHMMSS.npz

可视化录制:
    python visualize_recording.py --recording_path /home/pi-zero/Documents/see_to_reach_feel_to_insert/task/peg_in_hole_square_III/checkpoints_curriculum_fast_relative/recordings/recording_20260122_144925.npz --output video.mp4

配置选项:
=========
- RECORDING_ENABLED: 是否启用录制 (默认 True)
- RECORDING_FPS: 录制帧率 (默认与控制频率相同)
- SERL_PROPRIO_KEYS: 可选 proprio 状态 keys
- TACTILE_ENABLED: 是否启用触觉传感器
- TACTILE_INPUT_MODE: 触觉输入模式 ("both", "critic", "actor", "none")
"""
...
class AsyncSessionRecorder:
    """
    异步会话录制器 - 使用后台线程处理录制，避免阻塞控制循环

    录制内容:
    - timestamps: 时间戳 (秒)
    - tactile: 6D 触觉数据 [Fx, Fy, Fz, Mx, My, Mz]
    - wrist_2: wrist_2 相机图像 (压缩为 JPEG)
    - top: top 相机图像 (压缩为 JPEG)
    - ee_pose: 末端执行器位姿 (7D: xyz + quat)
    - actions: 执行的动作 (3D: xyz)
    - episode_ids: episode 标识
    - intervention_flags: 是否为人类干预

    特点:
    - record() 方法非阻塞，只将数据放入队列
    - 后台线程异步处理 JPEG 压缩和数据存储
    - 不影响主控制循环的时序

    保存格式: .npz (numpy 压缩格式)
    """

    def __init__(self, save_dir: str, max_frames: int = 100000, queue_size: int = 1000):
        """
        Args:
            save_dir: 保存目录
            max_frames: 最大录制帧数 (防止内存溢出)
            queue_size: 队列大小 (缓冲待处理的帧)
        """
    ...
    def _worker(self):
        ...
        while not self._stop_event.is_set() or not self._queue.empty():
            ...
            # 解包数据
            timestamp, tactile, wrist_2_img, top_img, ee_pose, action, episode_id, intervention, reward = data

            # 压缩图像为 JPEG (耗时操作，在后台线程执行)
            try:
                wrist_2_encoded = cv2.imencode('.jpg', wrist_2_img, [cv2.IMWRITE_JPEG_QUALITY, 80])[1].tobytes()
                top_encoded = cv2.imencode('.jpg', top_img, [cv2.IMWRITE_JPEG_QUALITY, 80])[1].tobytes()
            except Exception as e:
                print(f"[Recorder] JPEG encode error: {e}")
                wrist_2_encoded = b''
                top_encoded = b''
            ...
            with self._lock:
                ...
                self.tactile_data.append(tactile)
                self.wrist_2_images.append(wrist_2_encoded)
                self.top_images.append(top_encoded)
                ...
                if self.frame_count % 1000 == 0:
                    ...
                    print(f"[Recorder] {self.frame_count} frames processed (queue: {queue_size})")
    ...
    def start(self):
        ...
        print(f"[Recorder] Started async recording (max {self.max_frames} frames, queue size: {self._queue.maxsize})")
    ...
    def stop(self):
        ...
        if self._worker_thread is not None:
            print(f"[Recorder] Stopping... waiting for {self._queue.qsize()} frames in queue")
            self._queue.put(None)  # 发送停止信号
            self._worker_thread.join(timeout=10.0)
            if self._worker_thread.is_alive():
                print("[Recorder] Warning: worker thread did not finish in time")

        print(f"[Recorder] Stopped (enqueued: {self.enqueued_count}, processed: {self.frame_count})")
    ...
    def record(
        ...
        Args:
            tactile: 6D 触觉数据 [Fx, Fy, Fz, Mx, My, Mz]
            wrist_2_img: wrist_2 相机图像 (BGR, 任意尺寸)
            top_img: top 相机图像 (BGR, 任意尺寸)
            ee_pose: 末端执行器位姿 (7D 或更多)
            action: 执行的动作 (3D)
            intervention: 是否为人类干预
            reward: 奖励值
        """
        if not self.recording or self.enqueued_count >= self.max_frames:
            return
        ...
        # 复制数据（避免后续修改影响）
        tactile_copy = tactile.copy() if tactile is not None else np.zeros(6)
        wrist_2_copy = wrist_2_img.copy() if wrist_2_img is not None else np.zeros((480, 640, 3), dtype=np.uint8)
        top_copy = top_img.copy() if top_img is not None else np.zeros((480, 640, 3), dtype=np.uint8)
        ee_pose_copy = ee_pose[:7].copy() if ee_pose is not None else np.zeros(7)
        action_copy = action.copy() if action is not None else np.zeros(3)

        # 将数据放入队列 (非阻塞)
        data = (timestamp, tactile_copy, wrist_2_copy, top_copy, ee_pose_copy,
                action_copy, self.current_episode, intervention, reward)
    ...
    def save(self, filename: str = None) -> str:
        ...
        if remaining > 0:
            print(f"[Recorder] Waiting for {remaining} frames to be processed...")
            self._queue.join()
        ...
        with self._lock:
            ...
            np.savez_compressed(
                ...
                top_images=np.array(self.top_images, dtype=object),
...
# 保持向后兼容的别名
SessionRecorder = AsyncSessionRecorder
...
flags.DEFINE_string(
    "checkpoint_path",
    "/home/pi-zero/Documents/see_to_reach_feel_to_insert/task/peg_in_hole_square_III/checkpoints_curriculum_fast_relative",
...
class CurriculumConfig:
    ...
    # ==================== 录制配置 ====================
    RECORDING_ENABLED: bool = True  # 是否启用录制
    RECORDING_MAX_FRAMES: int = 100000  # 最大录制帧数
...
def actor(agent, data_store, intvn_data_store, train_config, serl_space_low, serl_space_high):
    ...
    def read_tactile():
        ...
        data = tactile_sensor.read_force_torque()
        if data is not None:
    ...
    # 初始化录制器 (异步版本)
    recorder = None
    if curriculum_config.RECORDING_ENABLED:
        recording_dir = os.path.join(FLAGS.checkpoint_path, "recordings")
        recorder = SessionRecorder(
            save_dir=recording_dir,
            max_frames=curriculum_config.RECORDING_MAX_FRAMES
        )
        print(f"[Init] Async session recorder initialized (save to: {recording_dir})")
    ...
    try:
        while not state.save_and_exit and not state.exit_flag:
            ...
            while not switched and not state.save_and_exit and not state.exit_flag:
                ...
                # 录制 DP 阶段数据 (异步，非阻塞)
                if recorder is not None and images:
                    tactile_data = read_tactile()
                    recorder.record(
                        tactile=tactile_data,
                        wrist_2_img=images.get("wrist_2"),
                        top_img=images.get("side"),  # DP 阶段使用 side 作为 top
                        ee_pose=robot_state["ee_6d"],
                        action=action[:3] if len(action) >= 3 else action,
                        intervention=False,  # DP 阶段没有人类干预
                        reward=0.0,
                    )
            ...
            while not state.end_serl and not state.save_and_exit and not state.exit_flag:
                ...
                while not state.end_serl and not state.save_and_exit and not state.exit_flag:
                    ...
                    # 录制数据 (异步，非阻塞)
                    # 使用 next_images (动作执行后的图像) 和 next_robot_state
                    if recorder is not None:
                        tactile_data = read_tactile()
                        recorder.record(
                            tactile=tactile_data,
                            wrist_2_img=next_images.get("wrist_2"),
                            top_img=next_images.get("side"),  # 使用 side 作为 top
...
def main(_):
    ...
    # 计算 state 维度基于 SERL_PROPRIO_KEYS
    def get_state_dim(proprio_keys):
        ...
        # 不再使用 dummy，返回实际维度 (可能为 0)
        return dim

    state_dim = get_state_dim(curriculum_config.SERL_PROPRIO_KEYS)
    use_proprio = state_dim > 0  # 只有在有 proprio 输入时才使用
    print(f"[Config] SERL_PROPRIO_KEYS: {curriculum_config.SERL_PROPRIO_KEYS}")
    print(f"[Config] State dimension: {state_dim}")
    print(f"[Config] use_proprio: {use_proprio}")
    ...
    # 当 use_proprio=False 时，不包含 "state" key
    ...
    # 直接调用 SACAgent.create_pixels，以便传入 use_proprio 参数
    agent: SACAgent = SACAgent.create_pixels(
        jax.random.PRNGKey(FLAGS.seed),
        custom_obs_space.sample(),
        custom_action_space.sample(),  # 使用 3D action space
        encoder_type=train_config.encoder_type,
        use_proprio=use_proprio,  # 根据 SERL_PROPRIO_KEYS 是否为空决定
