#!/usr/bin/env python3
"""
DP-SERL 课程学习训练脚本 (快速切换版本)

与 run_dp_serl_curriculum.py 的区别:
- 相机始终保持 640x480，DP 和 SERL 阶段无需切换相机
...
Terminal 1 (Learner):
    ...
    python run_dp_serl_curriculum_fast.py --learner --exp_name=peg_in_hole_square_III

Terminal 2 (Actor):
    export XLA_PYTHON_CLIENT_PREALLOCATE=false
    export XLA_PYTHON_CLIENT_MEM_FRACTION=.2
    python run_dp_serl_curriculum_fast.py --actor --exp_name=peg_in_hole_square_III --ip=localhost

默认 checkpoint 路径:
    task/peg_in_hole_square_III/checkpoints_curriculum_fast/
"""
...
flags.DEFINE_string(
    "checkpoint_path",
    "/home/pi-zero/Documents/see_to_reach_feel_to_insert/task/peg_in_hole_square_III/checkpoints_curriculum_fast",
...
class CurriculumConfig:
    ...
    SERL_LEARNER_SLEEP: float = 0.1  # 每次训练迭代后暂停时间 (秒), 0=无延迟
...
def actor(agent, data_store, intvn_data_store, train_config, serl_space_low, serl_space_high):
    ...
    # 原始 1280x720 的 crop: img[214:400, 636:799]
    # 转换到 640x480: 宽度 /2, 高度 *480/720 = *2/3
    # 新 crop: img[143:267, 318:400]
    image_crop = {
