...
python run_dp_serl_curriculum_fast_img-only_tactile-critic-6D+6D.py --learner --exp_name=peg_in_hole_square_III

Terminal 2 (Actor):
    export XLA_PYTHON_CLIENT_PREALLOCATE=false
    export XLA_PYTHON_CLIENT_MEM_FRACTION=.2
python run_dp_serl_curriculum_fast_img-only_tactile-critic-6D+6D.py --actor --exp_name=peg_in_hole_square_III --ip=localhost
默认 checkpoint 路径:
...
# 触觉历史缓冲区
# =============================================================================
# TactileHistoryBuffer 类已移除 - 改用 baseline + current 双 6D 模式
...
def create_asymmetric_sac_agent(
    ...
    tx = make_optimizer(learning_rate=3e-4)
...
flags.DEFINE_string(
    "checkpoint_path",
    "/home/pi-zero/Documents/see_to_reach_feel_to_insert/task/peg_in_hole_square_III/checkpoints_tactile_critic-6D+6D",
...
class CurriculumConfig:
    ...
    TACTILE_PORT: str = "/dev/ttyACM0"  # 触觉传感器串口
    TACTILE_SCALE_FACTOR: float = 0.1  # 力值标定系数 (1 LSB = 0.1N)

    # ==================== 触觉输入模式���新版：Baseline + Current）====================
    # Baseline: SERL 开始前抓取稳定后的触觉读数（6D）
    # Current:  SERL 运行时的当前触觉读数（6D）

    # Critic 输入配置
    TACTILE_CRITIC_USE_BASELINE: bool = True   # Critic 是否使用 baseline (6D)
    TACTILE_CRITIC_USE_CURRENT: bool = True    # Critic 是否使用 current (6D)

    # Actor 输入配置（通常不建议，保持纯视觉）
    TACTILE_ACTOR_USE_BASELINE: bool = False   # Actor 是否使用 baseline (6D)
    TACTILE_ACTOR_USE_CURRENT: bool = False    # Actor 是否使用 current (6D)

    # Baseline 采集配置
    TACTILE_BASELINE_WAIT: float = 1.0         # 抓取稳定后等待时间（秒）
    TACTILE_BASELINE_SAMPLES: int = 10         # Baseline 采样帧数（取平均）
    TACTILE_BASELINE_SAMPLE_INTERVAL: float = 0.1  # 采样间隔（秒）

    # 注：移除了 TACTILE_HISTORY_LENGTH，改用 baseline + current 双 6D 模式
...
def get_serl_observation(
    ...
    baseline_tactile: np.ndarray = None,
    current_tactile: np.ndarray = None,
    tactile_config: dict = None
) -> dict:
    """
    构建 SERL 观测 (使用 640x480 图像) - 新版：支持 baseline + current 触觉
    ...
    Args:
        ...
        baseline_tactile: 稳定抓取时的触觉读数 (6D) [Fx, Fy, Fz, Mx, My, Mz] (可选)
        current_tactile: 当前触觉读数 (6D) [Fx, Fy, Fz, Mx, My, Mz] (可选)
        tactile_config: 触觉配置字典 {'use_baseline': bool, 'use_current': bool} (可选)
    ...
    # 构建 state components 列表（按顺序）
    state_components = []

    # 1. 按照 proprio_keys 顺序添加基础状态
    for key in proprio_keys:
        if key == "relative_xyz":
            if relative_transformer is not None:
                state_components.append(relative_transformer.get_relative_xyz(robot_state["ee_6d"][:3]))
            else:
                state_components.append(robot_state["ee_6d"][:3])
        elif key == "tcp_pose":
            state_components.append(robot_state["ee_6d"])  # 6D: xyz + euler
        elif key == "tcp_vel":
            state_components.append(robot_state["tcp_vel"][:3])  # 3D 线速度
        elif key == "tcp_force":
            state_components.append(robot_state["force"])  # 3D
        elif key == "tcp_torque":
            state_components.append(robot_state["torque"])  # 3D
        elif key == "gripper_pose":
            state_components.append(np.array([robot_state["gripper_pos"]]))  # 1D
        # 跳过 "tactile" key，因为触觉现在单独处理

    # 2. 根据 tactile_config 添加触觉数据
    if tactile_config is not None:
        # Baseline 触觉
        if tactile_config.get('use_baseline', False):
            if baseline_tactile is not None:
                state_components.append(np.asarray(baseline_tactile, dtype=np.float32).flatten())
            else:
                state_components.append(np.zeros(6, dtype=np.float32))

        # Current 触觉
        if tactile_config.get('use_current', False):
            if current_tactile is not None:
                state_components.append(np.asarray(current_tactile, dtype=np.float32).flatten())
            else:
                state_components.append(np.zeros(6, dtype=np.float32))
    ...
    if len(state_components) > 0:
        state_vec = np.concatenate(state_components).astype(np.float32)
...
def learner(agent, replay_buffer, demo_buffer, train_config, wandb_logger=None):
    ...
    for step in tqdm.tqdm(
        ...
        if step % curriculum_config.SERL_LOG_PERIOD == 0 and wandb_logger:
            ...
            # 计算 Critic Disagreement（关键指标！）
            # 从 replay buffer 采样一个 batch 来计算
            eval_batch = next(replay_iterator)

            # 获取两个 critics 的 Q 值估计
            # agent.state.params 包含两个 critic 网络
            ...
            from jax import jit
            @jit
            def compute_disagreement(params, observations, actions):
                # 获取 critic 网络的输出
                # 你的 SAC agent 有 2 个 critics
                critic_1 = agent.critic.apply(
                    {"params": params["modules_critic"]["critic_0"]},
                    observations, actions
                )
                critic_2 = agent.critic.apply(
                    {"params": params["modules_critic"]["critic_1"]},
                    observations, actions
                )
                # 计算 disagreement
                disagreement = jnp.abs(critic_1 - critic_2).mean()
                q_mean = (critic_1 + critic_2).mean() / 2
                q_std = jnp.concatenate([critic_1, critic_2]).std()
                return disagreement, q_mean, q_std

            try:
                disagreement, q_mean, q_std = compute_disagreement(
                    agent.state.params, obs_eval, actions_eval
                )

                # 记录 Critic 统计
                wandb_logger.log({
                    "critic/disagreement_mean": float(disagreement),
                    "critic/q_mean": float(q_mean),
                    "critic/q_std": float(q_std),
                    "buffer/size": len(replay_buffer),
                    "buffer/demo_size": len(demo_buffer),
                }, step=step)
            except Exception as e:
                print(f"Warning: Failed to compute critic disagreement: {e}")
...
def actor(agent, data_store, intvn_data_store, train_config, serl_space_low, serl_space_high):
    ...
    if curriculum_config.TACTILE_ENABLED and TACTILE_AVAILABLE:
        ...
        if tactile_sensor.connect():
            ...
            if test_data is not None:
                print(f"  Tactile sensor OK. Initial: F=[{test_data[0]:.2f}, {test_data[1]:.2f}, {test_data[2]:.2f}] N")
            else:
    ...
    def collect_baseline_tactile(tactile_sensor, config):
        """
        采集 baseline 触觉读数（SERL 开始前抓取稳定后的触觉读数）

        Args:
            tactile_sensor: TactileSensor 实例
            config: 配置对象（包含 TACTILE_BASELINE_* 参数）
        ...
        baseline_tactile = np.mean(baseline_samples, axis=0).astype(np.float32)
        ...
        while not state.save_and_exit and not state.exit_flag:
            ...
            while not state.end_serl and not state.save_and_exit and not state.exit_flag:
                ...
                while not state.end_serl and not state.save_and_exit and not state.exit_flag:
                    ...
                    # 获取观测 (使用相对坐标 + baseline/current 触觉)
                    # 注：存储的 observation 使用 Critic 的配置（最大state维度）
                    # Actor 会自动忽略 state（如果 use_proprio_actor=False）
                    obs = get_serl_observation(
                        ...
                        tactile_config={
                            'use_baseline': curriculum_config.TACTILE_CRITIC_USE_BASELINE,
                            'use_current': curriculum_config.TACTILE_CRITIC_USE_CURRENT,
                        }
                    )
                    ...
                    # 读取下一个时刻的触觉数据 (6D)
                    next_current_tactile = None
                    if tactile_sensor is not None:
                        next_current_tactile = tactile_sensor.read_force_torque()

                    # 获取下一个观测 (使用相对坐标 + baseline/current 触觉)
                    # 注：存储的 observation 使用 Critic 的配置（最大state维度）
                    next_obs = get_serl_observation(
                        ...
                        relative_transformer=relative_transformer,
                        baseline_tactile=baseline_tactile,
                        current_tactile=next_current_tactile,
                        tactile_config={
                            'use_baseline': curriculum_config.TACTILE_CRITIC_USE_BASELINE,
                            'use_current': curriculum_config.TACTILE_CRITIC_USE_CURRENT,
                        }
                    ...
                    tactile_str = ""
                    if current_tactile is not None:
                        f_mag = np.linalg.norm(current_tactile[:3])
...
def main(_):
    ...
    # 计算 state 维度基于 SERL_PROPRIO_KEYS + 触觉 (新版：baseline + current)
    def get_state_dim_for_network(proprio_keys, use_baseline, use_current):
        """
        根据 proprio_keys 和触觉配置计算 state 维度

        Args:
            proprio_keys: SERL_PROPRIO_KEYS 列表
            use_baseline: 是否使用 baseline 触觉 (6D)
            use_current: 是否使用 current 触觉 (6D)
        ...
        # 添加触觉维度
        if use_baseline:
            dim += 6  # baseline tactile (6D)
        if use_current:
            dim += 6  # current tactile (6D)

        return dim

    # 分别计算 actor 和 critic 的 state 维度
    actor_state_dim = get_state_dim_for_network(
        curriculum_config.SERL_PROPRIO_KEYS,
        curriculum_config.TACTILE_ACTOR_USE_BASELINE,
        curriculum_config.TACTILE_ACTOR_USE_CURRENT
    )
    critic_state_dim = get_state_dim_for_network(
        curriculum_config.SERL_PROPRIO_KEYS,
        curriculum_config.TACTILE_CRITIC_USE_BASELINE,
        curriculum_config.TACTILE_CRITIC_USE_CURRENT
    )
    ...
    print(f"[Config] SERL_PROPRIO_KEYS: {curriculum_config.SERL_PROPRIO_KEYS}")
    print(f"[Config] Tactile mode: Baseline + Current (新版)")
    print(f"[Config]   Critic: baseline={curriculum_config.TACTILE_CRITIC_USE_BASELINE}, current={curriculum_config.TACTILE_CRITIC_USE_CURRENT}")
    print(f"[Config]   Actor:  baseline={curriculum_config.TACTILE_ACTOR_USE_BASELINE}, current={curriculum_config.TACTILE_ACTOR_USE_CURRENT}")
    print(f"[Config] Actor state dimension: {actor_state_dim}")
    print(f"[Config] Critic state dimension: {critic_state_dim}")
    ...
    # 使用 critic_state_dim (最大维度)，因为存储的 observation 使用 critic 配置
    need_state_in_obs = use_proprio_actor or use_proprio_critic
    from gymnasium import spaces
    if need_state_in_obs and critic_state_dim > 0:
