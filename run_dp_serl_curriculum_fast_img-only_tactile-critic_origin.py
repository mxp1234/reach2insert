...
python run_dp_serl_curriculum_fast_img-only_tactile-critic.py --learner --exp_name=peg_in_hole_square_III

Terminal 2 (Actor):
    export XLA_PYTHON_CLIENT_PREALLOCATE=false
    export XLA_PYTHON_CLIENT_MEM_FRACTION=.2
python run_dp_serl_curriculum_fast_img-only_tactile-critic.py --actor --exp_name=peg_in_hole_square_III --ip=localhost
默认 checkpoint 路径:
...
class TactileHistoryBuffer:
    ...
    存储最近 N 帧的触觉数据，用于提供时序信息。
    - 新数据从前面插入，旧数据从后面移除
    - 如果历史不足，复制当前帧填充
    - 输出 flatten 后的历史向量 (history_length * 6D)
    """

    def __init__(self, history_length: int = 5, tactile_dim: int = 6):
        """
        Args:
            history_length: 历史帧数
            tactile_dim: 单帧触觉维度 (默认6D: Fx, Fy, Fz, Mx, My, Mz)
        """
        self.history_length = history_length
        self.tactile_dim = tactile_dim
        self.buffer = None  # shape: (history_length, tactile_dim)

    def reset(self):
        """重置缓冲区（新episode开始时调用）"""
        self.buffer = None

    def update(self, tactile_data: np.ndarray) -> np.ndarray:
        """
        更新缓冲区并返回历史向量

        Args:
            tactile_data: 当前帧触觉数据 (6D)

        Returns:
            历史向量 (history_length * tactile_dim,) - flatten后的结果
            顺序: [最新帧, 次新帧, ..., 最旧帧]
        """
        if tactile_data is None:
            tactile_data = np.zeros(self.tactile_dim, dtype=np.float32)

        tactile_data = np.asarray(tactile_data, dtype=np.float32).flatten()

        if self.buffer is None:
            # 初始化：复制当前帧填充整个历史
            self.buffer = np.tile(tactile_data, (self.history_length, 1))
        else:
            # 移除最旧帧，插入最新帧
            self.buffer = np.roll(self.buffer, shift=1, axis=0)
            self.buffer[0] = tactile_data

        # 返回 flatten 的历史向量
        return self.buffer.flatten().astype(np.float32)

    def get_history(self) -> np.ndarray:
        """获取当前历史向量（不更新）"""
        if self.buffer is None:
            return np.zeros(self.history_length * self.tactile_dim, dtype=np.float32)
        return self.buffer.flatten().astype(np.float32)

    @property
    def output_dim(self) -> int:
        """输出向量维度"""
        return self.history_length * self.tactile_dim
...
flags.DEFINE_string(
    "checkpoint_path",
    "/home/pi-zero/Documents/see_to_reach_feel_to_insert/task/peg_in_hole_square_III/checkpoints_tactile_critic",
...
class CurriculumConfig:
    ...
    # ==================== 触觉输入模式 ====================
    # "both"   : Actor 和 Critic 都使用触觉
    # "critic" : 只有 Critic 使用触觉 (推荐 - Actor 纯视觉，Critic 有触觉辅助)
    # "actor"  : 只有 Actor 使用触觉
    # "none"   : 都不使用触觉 (但仍会采集和显示)
    TACTILE_INPUT_MODE: str = "critic"

    # ==================== 触觉历史帧配置 ====================
    TACTILE_HISTORY_LENGTH: int = 5  # 历史帧数 (1 = 只用当前帧)
    # 触觉维度 = 6D * history_length
    # 例如: history=5 → 30D输入
...
def get_serl_observation(
    ...
    Args:
        ...
        tactile_data: 触觉传感器数据 [Fx, Fy, Fz, Mx, My, Mz] (可选)
    ...
    # 触觉传感器数据 (历史向量)
    if "tactile" in proprio_keys:
        if tactile_data is not None:
            # tactile_data 应该已经是历史向量 (history_length * 6D)
            state_dict["tactile"] = np.asarray(tactile_data, dtype=np.float32).flatten()
        else:
            # 默认零向量 (需要知道历史长度，这里假设调用者传入正确维度)
            state_dict["tactile"] = np.zeros(6, dtype=np.float32)  # fallback
...
def actor(agent, data_store, intvn_data_store, train_config, serl_space_low, serl_space_high):
    ...
    if curriculum_config.TACTILE_ENABLED and TACTILE_AVAILABLE:
        ...
        if tactile_sensor.connect():
            ...
            if test_data is not None:
                ...
                # 创建历史缓冲区
                tactile_history_buffer = TactileHistoryBuffer(
                    history_length=curriculum_config.TACTILE_HISTORY_LENGTH,
                    tactile_dim=6
                )
                print(f"  Tactile history buffer: {curriculum_config.TACTILE_HISTORY_LENGTH} frames × 6D = {tactile_history_buffer.output_dim}D")
            else:
    ...
    def read_tactile_history():
        """读取触觉传感器并更新历史缓冲区，返回历史向量"""
        if tactile_sensor is None or tactile_history_buffer is None:
            return None
        raw_data = tactile_sensor.read_force_torque()
        # 更新缓冲区并返回历史向量
        return tactile_history_buffer.update(raw_data)

    def reset_tactile_history():
        """重置触觉历史缓冲区（新episode开始时调用）"""
        if tactile_history_buffer is not None:
            tactile_history_buffer.reset()
        ...
        while not state.save_and_exit and not state.exit_flag:
            ...
            while not state.end_serl and not state.save_and_exit and not state.exit_flag:
                ...
                while not state.end_serl and not state.save_and_exit and not state.exit_flag:
                    ...
                    # 读取触觉数据 (历史向量)
                    tactile_data = read_tactile_history()
                    ...
                    # 读取下一个时刻的触觉数据 (历史向量)
                    next_tactile_data = read_tactile_history()
                    ...
                    next_obs = get_serl_observation(
                        ...
                        proprio_keys=curriculum_config.SERL_PROPRIO_KEYS,
                        relative_transformer=relative_transformer,
                        tactile_data=next_tactile_data
                    )
                    ...
                    pos = target_pose[:3]
                    # 显示状态，可选显示触觉信息 (从历史向量提取当前帧)
                    tactile_str = ""
                    if tactile_data is not None and len(tactile_data) >= 6:
                        # 历史向量的前6个元素是最新帧
                        current_tactile = tactile_data[:6]
                        f_mag = np.sqrt(current_tactile[0]**2 + current_tactile[1]**2 + current_tactile[2]**2)
...
def main(_):
    ...
    def get_state_dim(proprio_keys, tactile_history_length=1):
        ...
        key_dims = {
            ...
            "tcp_torque": 3,
            "gripper_pose": 1,
            # tactile 特殊处理：6D * history_length
        }
        for key in proprio_keys:
            if key == "tactile":
                dim += 6 * tactile_history_length  # 6D * 历史帧数
            else:
                dim += key_dims.get(key, 0)
        return dim

    tactile_history_length = curriculum_config.TACTILE_HISTORY_LENGTH
    state_dim = get_state_dim(curriculum_config.SERL_PROPRIO_KEYS, tactile_history_length)
    use_proprio = state_dim > 0
    print(f"[Config] SERL_PROPRIO_KEYS: {curriculum_config.SERL_PROPRIO_KEYS}")
    print(f"[Config] TACTILE_HISTORY_LENGTH: {tactile_history_length}")
    print(f"[Config] State dimension: {state_dim}")
    print(f"[Config] use_proprio: {use_proprio}")

    # 根据 TACTILE_INPUT_MODE 确定 actor/critic 是否使用 proprio
    tactile_mode = curriculum_config.TACTILE_INPUT_MODE
    if tactile_mode == "both":
        use_proprio_actor = use_proprio
        use_proprio_critic = use_proprio
    elif tactile_mode == "critic":
        # Critic 使用 state，Actor 不使用 (纯视觉策略)
        use_proprio_actor = False
        use_proprio_critic = use_proprio
    elif tactile_mode == "actor":
        # Actor 使用 state，Critic 不使用
        use_proprio_actor = use_proprio
        use_proprio_critic = False
    else:  # "none"
        use_proprio_actor = False
        use_proprio_critic = False

    print(f"[Config] TACTILE_INPUT_MODE: {tactile_mode}")
    ...
    if need_state_in_obs and state_dim > 0:
