...
class SessionRecorder:
    ...
    def stop(self):
        """停止录制"""
        self.recording = False
        print(f"[Recorder] Stopped recording ({self.frame_count} frames recorded)")
...
flags.DEFINE_boolean("debug", True, "Debug mode (disable wandb)")
...
def actor(agent, data_store, intvn_data_store, train_config, serl_space_low, serl_space_high):
    ...
    if curriculum_config.RECORDING_ENABLED:
        ...
        print(f"[Init] Session recorder initialized (save to: {recording_dir})")
    ...
    try:
        while not state.save_and_exit and not state.exit_flag:
            ...
            if tactile_sensor is not None:
                print("  Calibrating tactile sensor...")
                if tactile_sensor.calibrate():
                    print("  Tactile sensor calibrated successfully")
                else:
                    print("  Warning: Tactile sensor calibration failed")
            ...
            while not switched and not state.save_and_exit and not state.exit_flag:
                ...
                if recorder is not None:
                    ...
                    recorder.record(
                        ...
                        top_img=rec_images.get("side"),  # DP 阶段使用 side 作为 top
